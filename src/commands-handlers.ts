...
export async function handleConnectCommand(msg: TelegramBot.Message): Promise<void> {
    ...
    // Create an instance of TonConnectStorage
    ...
    const unsubscribe = connector.onStatusChange(async wallet => {
        if (wallet) {
            ...
            // Increment the wallet connection count
            await storage.incrementWalletConnectionCount();
...
export async function handleDisconnectCommand(msg: TelegramBot.Message): Promise<void> {
    ...
    await storage.decrementWalletConnectionCount();
...
export async function handleGameCommand(msg: TelegramBot.Message): Promise<void> {
    const chatId = msg.chat.id;
    const gameUrl = `${process.env.SERVER_URL}/unity-game/index.html`; // URL of the hosted Unity WebGL game

    const connector = getConnector(chatId);

   await bot.sendGame(chatId, "theHarry")
